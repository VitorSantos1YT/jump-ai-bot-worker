// FASE 4 - VERSÃO FINAL COM PREVIEW E PROTEÇÃO CONTRA TIMEOUT
...
export default {
  ...
  async handleTelegramWebhook(request, env, ctx) {
    ...
    try {
      ...
      if (payload.message) {
        // Envia o processamento pesado para segundo plano
  ...
  async safeEditFileWithAI(env, repo, filePath, instruction) {
    ...
    // É preciso buscar o SHA do arquivo na nova branch antes de atualizar
    const fileInNewBranch = await this.getGithubFileContent(env, repo, filePath, true, branchName);
    if (fileInNewBranch.error) return `❌ Erro ao acessar o arquivo no novo ambiente de teste: ${fileInNewBranch.message}`;

    const commitResult = await this.updateGithubFile(env, repo, filePath, newContent, fileInNewBranch.sha, `feat: edita ${filePath} via IA`, branchName);
